...
## Test/coverage outputs explained

- **Surefire reports** (`target/surefire-reports`): per-test execution results (pass/fail, stack traces, timing) generated by Maven Surefire.
- **JaCoCo report** (`target/site/jacoco/index.html`): code coverage report (line/branch/method/class coverage) generated during `verify`.

## CI pipeline and Jenkins

- Current pipeline tool: **GitHub Actions** (`.github/workflows/ci.yml`).
- Jenkins alternative is now included as `Jenkinsfile`. You can use either tool depending on your course/lab requirement.
